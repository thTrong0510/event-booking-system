 ...
 */
@Service
public class LoginValidator implements ConstraintValidator<RegisterChecked, ReqUserLoginDTO> {

    @Override
    public boolean isValid(ReqUserLoginDTO user, ConstraintValidatorContext context) {
        boolean valid = true;
        if (!user.getPassword().equals(user.getEmail().isBlank())) {
        ...
        if (!user.getPassword().equals(user.getPassword().isBlank())) {
